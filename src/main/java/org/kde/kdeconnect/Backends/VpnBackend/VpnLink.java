...
public class VpnLink extends BaseLink {

    private IoSession session = null;

    public void disconnect() {
        if (session == null) return;
        //Log.i("VpnLink", "Disconnect: "+session.getRemoteAddress().toString());
        session.close(true);
    }

    public VpnLink(IoSession session, String deviceId, BaseLinkProvider linkProvider) {
        ...
        try {
            ...
            while(!success) {
                try {
                    ...
                    success = true;
                    Log.i("VpnLink", "ServerSocket is bound to address "+candidateServer.getInetAddress().toString());
                } catch(Exception e) {
                    Log.e("VpnLink", "Exception opening serversocket: "+e);
                    tcpPort++;
                    if (tcpPort >= 1764) {
                        Log.e("VpnLink", "No more ports available");
                        return null;
                    }
                }
            }
            ...
            Thread thread = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        int bytesRead;
                        Log.e("VpnLink","Beginning to send payload");
                        while ((bytesRead = stream.read(buffer)) != -1) {
                            //Log.e("ok",""+bytesRead);
                            socket.write(buffer, 0, bytesRead);
                        }
                        Log.e("VpnLink","Finished sending payload");
                    } catch(Exception e) {
                        e.printStackTrace();
                        Log.e("VpnLink", "Exception with payload upload socket");
                    } finally {
                        if (socket != null) {
    ...
    public boolean sendPackage(final NetworkPackage np) {
        ...
        if (session == null) {
            Log.e("VpnLink", "sendPackage failed: not yet connected");
            return false;
        }
        ...
        try {
            ...
            e.printStackTrace();
            Log.e("VpnLink", "sendPackage exception");
            return false;
        }
    ...
    public boolean sendPackageEncrypted(NetworkPackage np, PublicKey key) {
        ...
        try {
            ...
            e.printStackTrace();
            Log.e("VpnLink", "sendPackageEncrypted exception");
            return false;
        }
    ...
    public void injectNetworkPackage(NetworkPackage np) {
        ...
        if (np.hasPayloadTransferInfo()) {
            ...
            try {
                ...
                e.printStackTrace();
                Log.e("VpnLink", "Exception connecting to payload remote socket");
            }
