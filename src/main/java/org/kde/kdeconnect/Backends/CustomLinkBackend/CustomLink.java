...
public class CustomLink extends BaseLink {

    private IoSession session = null;

    public void disconnect() {
        if (session == null) return;
        //Log.i("CustomLink", "Disconnect: "+session.getRemoteAddress().toString());
        session.close(true);
    }

    public CustomLink(IoSession session, String deviceId, BaseLinkProvider linkProvider) {
        ...
        try {
            ...
            while(!success) {
                try {
                    ...
                    Log.e("CustomLink", "Exception openning serversocket: "+e);
                    tcpPort++;
                    if (tcpPort >= 1764) {
                        Log.e("CustomLink", "No more ports available");
                        return null;
                    }
                }
            }
            ...
            Thread thread = new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        int bytesRead;
                        Log.e("CustomLink","Beginning to send payload");
                        while ((bytesRead = stream.read(buffer)) != -1) {
                            //Log.e("ok",""+bytesRead);
                            socket.write(buffer, 0, bytesRead);
                        }
                        Log.e("CustomLink","Finished sending payload");
                    } catch(Exception e) {
                        e.printStackTrace();
                        Log.e("CustomLink", "Exception with payload upload socket");
                    } finally {
                        if (socket != null) {
    ...
    public boolean sendPackage(final NetworkPackage np) {
        ...
        if (session == null) {
            Log.e("CustomLink", "sendPackage failed: not yet connected");
            return false;
        }
        ...
        try {
            ...
            e.printStackTrace();
            Log.e("CustomLink", "sendPackage exception");
            return false;
        }
    ...
    public boolean sendPackageEncrypted(NetworkPackage np, PublicKey key) {
        ...
        try {
            ...
            e.printStackTrace();
            Log.e("CustomLink", "sendPackageEncrypted exception");
            return false;
        }
    ...
    public void injectNetworkPackage(NetworkPackage np) {
        ...
        if (np.hasPayloadTransferInfo()) {
            ...
            try {
                ...
                e.printStackTrace();
                Log.e("CustomLink", "Exception connecting to payload remote socket");
            }
